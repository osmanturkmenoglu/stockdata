...
if st.button("Get Data") and symbol:
    try:
        ...
        # Check if the selected column has at least one non-NaN value
        if df_plot[column_to_plot].notna().any():
            # Plotting the selected data
            st.line_chart(df_plot[column_to_plot])
        else:
            st.warning(f"The selected column '{column_to_plot}' does not contain numeric data that can be plotted.")
